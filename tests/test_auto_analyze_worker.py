...
class TestFloorClosedBar:
    ...
    NOTE: floor_closed_bar returns the close_time of the last closed bar.
    For a bar with open_time T and timeframe tf_ms: close_time = T + tf_ms.
    The current bar's open_time equals the last closed bar's close_time.
    """
    ...
    def test_floor_closed_bar_1m(self):
        ...
        # At 12:00:30, current bar started at 12:00:00, last closed bar closed at 12:00:00
        now_ms = 1700000030000  # arbitrary timestamp + 30 seconds
        last_closed = floor_closed_bar(now_ms, tf_ms)

        # Expected: current bar's open_time is the last closed bar's close_time
        current_bar_start = (now_ms // tf_ms) * tf_ms
        expected_last_closed_close = current_bar_start

        assert last_closed == expected_last_closed_close

    def test_floor_closed_bar_1h(self):
        """Test 1-hour timeframe."""
        tf_ms = TIMEFRAME_TO_MS["1h"]

        # At 14:30:00, current bar started at 14:00:00, last closed bar closed at 14:00:00
    ...
    def test_floor_closed_bar_near_boundary(self):
        ...
        # Should use previous bar's close_time because we're within tolerance
        expected_last_closed_close = current_bar_start - tf_ms
        assert last_closed == expected_last_closed_close
    ...
    def test_floor_closed_bar_3h(self):
        ...
        # At 15:30:00, current 3h bar started at 15:00:00, last closed bar closed at 15:00:00
    ...
    def test_floor_closed_bar_all_timeframes(self):
        ...
        for tf_name, tf_ms in TIMEFRAME_TO_MS.items():
            ...
            # Verify last_closed is before or equal to now
            assert last_closed <= now_ms

            # Verify last_closed is aligned to tf_ms
            assert last_closed % tf_ms == 0

            # Verify it equals current bar's open_time (close_time of last closed bar)
            current_bar_start = (now_ms // tf_ms) * tf_ms
            assert last_closed == current_bar_start
